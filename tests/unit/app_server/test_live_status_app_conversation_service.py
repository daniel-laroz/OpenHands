...
class TestLiveStatusAppConversationService:
    ...
    @pytest.mark.asyncio
    async def test_start_app_conversation_limit_reached_auto_pause_false_new_sandbox(
        self,
    ):
        """Test starting a new conversation raises 429 when limit reached and auto_pause is False."""
        from openhands.app_server.errors import MaxSandboxLimitReachedError

        # Arrange: No sandbox_id, auto_pause_existing=False
        request = AppConversationStartRequest(
            title='Test limit', auto_pause_existing=False
        )

        # Mock the sandbox service to raise the limit error
        self.mock_sandbox_service.raise_if_sandbox_limit_reached = AsyncMock(
        ...
        # Verify the limit check was explicitly called
        self.mock_sandbox_service.raise_if_sandbox_limit_reached.assert_called_once()
        # Verify it didn't even attempt to start/get a sandbox
        self.mock_sandbox_service.start_sandbox.assert_not_called()
        self.mock_sandbox_service.resume_sandbox.assert_not_called()

    @pytest.mark.asyncio
    async def test_start_app_conversation_limit_reached_auto_pause_false_paused_sandbox(
        self,
    ):
        """Test resuming a paused conversation raises 429 when limit reached and auto_pause is False."""
        from openhands.app_server.errors import MaxSandboxLimitReachedError

        # Arrange: Providing a sandbox_id, auto_pause_existing=False
        sandbox_id = 'test-paused-sandbox-id'
        request = AppConversationStartRequest(
            title='Test limit paused', sandbox_id=sandbox_id, auto_pause_existing=False
        )

        # Mock the retrieved sandbox as PAUSED
        self.mock_sandbox.status = SandboxStatus.PAUSED
        self.mock_sandbox_service.get_sandbox = AsyncMock(
            return_value=self.mock_sandbox
        )
        ...
        # Verify we fetched the sandbox info first, then checked the limit
        self.mock_sandbox_service.get_sandbox.assert_called_once_with(sandbox_id)
        self.mock_sandbox_service.raise_if_sandbox_limit_reached.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_start_app_conversation_limit_reached_auto_pause_false_running_sandbox(
        self,
    ):
        """Test attaching to a running conversation bypasses limit checks even if auto_pause is False."""
        # Arrange: Providing a sandbox_id, auto_pause_existing=False
        sandbox_id = 'test-running-sandbox-id'
        request = AppConversationStartRequest(
            title='Test limit running', sandbox_id=sandbox_id, auto_pause_existing=False
        )
        ...
        async_gen = self.service._start_app_conversation(request)

        task = await async_gen.__anext__()

        assert task is not None
        self.mock_sandbox_service.get_sandbox.assert_called_once_with(sandbox_id)
        self.mock_sandbox_service.raise_if_sandbox_limit_reached.assert_not_called()
